...
import { LEISURE_ITEMS } from '../../mock/leisureMockData';
import { C, MAX_WIDTH } from '../../styles/tokens';

export default function LeisurePage() {
  const [params, setSearchParams] = useSearchParams();
  const selectedCategory = params.get('category') || '';
  const categories = Array.from(new Set(LEISURE_ITEMS.map((item) => item.category)));
  // TODO(back-end): 레저/티켓 상품 목록, 지역, 즉시사용 여부 응답으로 교체한다.

  const filteredItems = LEISURE_ITEMS.filter((item) => !selectedCategory || item.category === selectedCategory);
    ...
    <div style={s.page}>
      ...
      <section style={s.section}>
        <div style={s.inner}>
          <div style={s.headRow}>
            <div>
              <p style={s.resultText}>{selectedCategory ? `${selectedCategory} 상품` : '추천 레저·티켓'}</p>
              <p style={s.resultCount}>{filteredItems.length}개 상품</p>
            </div>
            <Link to="/lodgings" style={s.secondaryBtn}>숙소 보기</Link>
          </div>

          <div style={s.chipWrap}>
            <button type="button" style={{ ...s.chip, ...(selectedCategory ? null : s.chipActive) }} onClick={() => updateCategory('')}>전체</button>
            {categories.map((category) => (
              <button key={category} type="button" style={{ ...s.chip, ...(selectedCategory === category ? s.chipActive : null) }} onClick={() => updateCategory(category)}>
                {category}
              </button>
            ))}
          </div>
          ...
          <div style={s.grid}>
            {filteredItems.map((item) => (
              <article key={item.leisureId} style={s.card}>
                <img src={item.imageUrl} alt={item.title} style={s.image} />
                <div style={s.body}>
                  <div style={s.topRow}>
                    <span style={s.badge}>{item.category}</span>
                    <span style={{ ...s.instantBadge, ...(item.instantUse ? s.instantActive : s.instantMuted) }}>
                      {item.instantUse ? '즉시 사용 가능' : '예약 확정 후 사용'}
                    </span>
                  </div>
                  <h2 style={s.cardTitle}>{item.title}</h2>
                  <p style={s.cardMeta}>{item.region}</p>
                  <div style={s.bottomRow}>
                    <p style={s.price}>최저 {item.priceFrom.toLocaleString()}원</p>
                    <Link to="/support" style={s.cardBtn}>상품 문의</Link>
                  </div>
                </div>
              </article>
            ))}
...
const s = {
  ...
  instantBadge: { display: 'inline-flex', padding: '6px 9px', borderRadius: '999px', fontSize: '11px', fontWeight: '800' },
  instantActive: { background: '#ECFDF5', color: '#047857' },
  instantMuted: { background: '#F4F4F5', color: '#52525B' },
