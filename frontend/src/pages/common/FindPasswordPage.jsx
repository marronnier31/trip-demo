...
export default function FindPasswordPage() {
  ...
  const handleSubmit = (event) => {
    ...
    setResult('가입된 계정이 확인되면 비밀번호 재설정 안내를 받을 수 있는 흐름으로 연결됩니다.');
  };
    ...
    <div style={s.page}>
      ...
      <div style={s.panel}>
        <div style={s.brandCol}>
          ...
          <h1 style={s.brandTitle}>비밀번호 찾기</h1>
          <p style={s.brandDesc}>가입 이메일과 연락처를 확인해 비밀번호 재설정 단계로 넘어갈 수 있게 준비합니다.</p>
          <div style={s.guideBox}>
            <p style={s.guideTitle}>현재 기준</p>
            <p style={s.guideText}>실제 재설정 링크 발송 없이, 설계 흐름에 맞는 프론트 화면만 먼저 구성합니다.</p>
          </div>
        </div>

        <div style={s.formCol}>
          <h2 style={s.formTitle}>본인 확인</h2>
          <p style={s.formDesc}>이메일과 전화번호를 입력해 주세요.</p>
          ...
          <form onSubmit={handleSubmit} style={s.form}>
            ...
            <button type="submit" style={s.submitBtn}>재설정 안내 받기</button>
          </form>
